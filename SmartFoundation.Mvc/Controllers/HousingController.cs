    ...
    HelpText = "اكتب أحرف عربية فقط"},
        ...
        public async Task<IActionResult> BuildingClass()
        {
                ...
                if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    if (ds != null && ds.Tables.Count > 0)
                    {
                        ...
                        var headerMap = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase)
                        {
                            ...
                            ["buildingClassName_A"] = "اسم المبنى بالعربي",
                            ["buildingClassName_E"] = "اسم المبنى بالانجليزي",
            ...
            var addFields = new List<FieldConfig>
            {
                ...
                // your custom textboxes
                new FieldConfig { Name = "p01", Label = "اسم الفئة بالعربي", Type = "arabictext", ColCss = "3" , Required = true,TextMode = "arabic", InputPattern = @"^[\u0621-\u064A\u0640\s]+$",
    HelpText = "اكتب أحرف عربية فقط"},
