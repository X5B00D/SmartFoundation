...
create PROCEDURE [Housing].[HousingResidentSP_2026_08_26] 
(
      @Action                               NVARCHAR(200)
...
------------------------------------------------------------
-- الشرط الثاني:
-- حالة المنزل التي سبقت التخصيص الحالي يجب أن تكون:
-- 5 أو 39 أو 41 أو 42
------------------------------------------------------------
SELECT TOP (1)
...
WHERE previousAction.buildingDetailsID_FK =
  ...
  -- استبعاد الحركات التي لا تخص حالة المنزل
  AND previousAction.buildingActionTypeID_FK IS NOT NULL
ORDER BY previousAction.buildingActionID DESC;

IF @BuildingStateBeforeAssign NOT IN (5, 39, 41, 42)
BEGIN
