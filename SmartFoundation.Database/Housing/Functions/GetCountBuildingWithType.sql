	...
	DECLARE @buildingType nvarchar(100)
	 SET @Result = 
					(SELECT count(*) 'Building Type'
							...
							INNER JOIN KFMC.Housing.BuildingType bt ON bd.buildingTypeID_FK = bt.buildingTypeID
							WHERE bt.buildingTypeActive = 1 AND bt.buildingTypeID = @buildingTypeID

					)
	SET @buildingType = (SELECT TOP(1) buildingTypeName_A FROM KFMC.Housing.BuildingType WHERE buildingTypeID = @buildingTypeID)
